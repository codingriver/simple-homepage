...
const proxyParamsSimple = path.resolve(__dirname, '../../../nginx-conf/proxy-params-simple.conf');
const proxyParamsFull = path.resolve(__dirname, '../../../nginx-conf/proxy-params-full.conf');
...
test('settings proxy params mode switches between simple and full', async ({ page }) => {
  ...
  // switch to full
  await page.locator('input#ppm_full').check();
  await page.locator('#proxy-params-mode-form').getByRole('button', { name: '保存模式' }).click();
  await expect(page.locator('.alert-success')).toContainText('模式已保存');
  ...
  await page.goto('/admin/settings.php#nginx');
  await page.locator('input#ppm_simple').check();
  await page.locator('#proxy-params-mode-form').getByRole('button', { name: '保存模式' }).click();
  await expect(page.locator('.alert-success')).toContainText('模式已保存');
