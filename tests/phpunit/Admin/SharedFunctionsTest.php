...
final class SharedFunctionsTest extends TestCase
{
    ...
    public function testBackupCollectPayloadContainsCoreSections(): void
    {
        save_config(['site_name' => '后台管理']);
        $payload = backup_collect_payload('export');
        $this->assertArrayHasKey('created_at', $payload);
        $this->assertArrayHasKey('trigger', $payload);
        $this->assertArrayHasKey('config', $payload);
        $this->assertArrayHasKey('scheduled_tasks', $payload);
        $this->assertArrayHasKey('dns_config', $payload);
        $this->assertArrayHasKey('ddns_tasks', $payload);
    ...
    {
        $source = DATA_DIR . '/trash_test_delete_dir';
        @mkdir($source, 0750, true);
        file_put_contents($source . '/x.txt', 'delete me');
        $move = trash_move('local', $source);
        $this->assertTrue($move['ok'] ?? false);
