...
修改代码/文档/部署前，请先阅读 `docs/协作RULES.md`。核心原则：
- **禁止凭经验下结论**：证据不足时必须明确写“不确定，需要先检查”。
- **禁止把“已修改”说成“已完成”**：必须区分“已修改 / 已验证 / 未验证 / 阻塞项”。
- **复杂任务必须先定边界**：说明目标、影响范围、前置条件、验证方式。
- **全量任务必须扫描->列清单->修改->复扫**：未做复扫前不得说“全部完成”。
