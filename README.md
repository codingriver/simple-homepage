...
- **备份与恢复**：本地手动备份、导出导入、WebDAV 手动上传/下载/恢复、远端保留最新 10 份
...
1. 本地支持手动 **创建**、**下载**、**恢复**、**删除**，本地最多保留 20 份。
2. 可配置一个可信 WebDAV 目标，由管理员手动执行 **备份到 WebDAV**、下载到本地、恢复和删除。
3. WebDAV 默认关闭 SSRF 防护、TLS 和认证，不创建自动/定时任务；可按目标能力手动开启这些选项。
4. 远端保存未加密 JSON，可能包含 DNS/DDNS、Webhook 和任务脚本中的敏感配置；每次上传成功后仅保留当前实例最新 10 份。
...
├── backup_webdav.json      # WebDAV 手动备份连接配置
├── backups/                # 本地备份快照
│   ├── jobs/               # WebDAV 后台任务状态和日志
│   └── tmp/                # WebDAV 下载/恢复临时文件
├── logs/                   # 各类日志
├── tasks/                  # 计划任务脚本和日志
