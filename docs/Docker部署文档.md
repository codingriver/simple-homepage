...
后台「备份与恢复」支持本地手动备份，以及管理员手动操作的 WebDAV 上传、下载和恢复。WebDAV 不创建定时任务，默认关闭 SSRF 防护、TLS 和认证；远端保存未加密 JSON，并固定保留当前实例最新 10 份。若备份包含 DNS/DDNS 等敏感配置，请只连接可信 WebDAV 目标。

完整数据目录仍建议定期在宿主机执行离线备份：
