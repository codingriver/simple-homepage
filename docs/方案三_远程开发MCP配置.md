...
```
电脑A：安装了 Cursor IDE
服务器B：192.168.2.2（代码、Docker、simple-homepage 容器都在这里）
...
- 确认三个 Server 状态为绿色
- 测试：在聊天框输入「列出 /volume3/storage/docker/simple-homepage 目录」
...
电脑A Cursor
  └── 电脑A MCP Server
        └── ssh user@192.168.2.2 'docker exec simple-homepage ...'
```
...
server.setRequestHandler('tools/list', async () => ({
  tools: [
    {
      name: 'docker_exec',
      description: '在 simple-homepage 容器内执行命令',
      inputSchema: {
        type: 'object',
...
server.setRequestHandler('tools/call', async (request) => {
  ...
  if (name === 'docker_exec') {
    const { stdout, stderr } = await execAsync(
      `docker exec simple-homepage sh -c ${JSON.stringify(args.command)}`
    );
    return { content: [{ type: 'text', text: stdout || stderr }] };
  }

  if (name === 'sync_file') {
    const cmd = `base64 ${args.host_path} | docker exec -i simple-homepage sh -c 'base64 -d > ${args.container_path}'`;
    await execAsync(cmd);
    const h1 = (await execAsync(`md5sum ${args.host_path}`)).stdout.split(' ')[0];
    const h2 = (await execAsync(`docker exec simple-homepage md5sum ${args.container_path}`)).stdout.split(' ')[0];
    const ok = h1 === h2;
    return { content: [{ type: 'text', text: ok ? `✅ 同步成功 MD5=${h1}` : `❌ 同步失败 host=${h1} container=${h2}` }] };
  }

  if (name === 'reset_nav') {
    await execAsync('docker exec simple-homepage rm -f /var/www/nav/data/.installed /var/www/nav/data/users.json /var/www/nav/data/config.json /var/www/nav/data/sites.json');
...
项目环境：
- 容器名：simple-homepage
- 源码路径：/volume3/storage/docker/simple-homepage/simple-homepage
- 容器内路径：/var/www/nav
...
4. 测试脚本：python3 /volume3/storage/docker/simple-homepage/http_test.py
```
...
# 3. 创建 MCP 配置（服务器B上）
mkdir -p /volume3/storage/docker/simple-homepage/.cursor
cat > /volume3/storage/docker/simple-homepage/.cursor/mcp.json << 'MCPEOF'
{
  "mcpServers": {
    "shell": {
      "command": "npx",
...
# 4. 在 Cursor 中打开项目目录
# File -> Open Folder -> /volume3/storage/docker/simple-homepage
